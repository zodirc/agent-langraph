...
def test_mission_decide_keeps_forced_pause_with_execution_grant(base_state, monkeypatch):
    import app.nodes.mission_decide_node as mod
    ...
    state = merge_state(
        ...
        input_payload={
            ...
            "mission_intervention": {"action": "pause", "force": True},
        },
    )
    result = mission_decide_node(state)
    assert result["step_decision"]["action"] == "pause"
