...
class Settings:
    ...
    def __init__(self, config_path: str | None = None) -> None:
        ...
        self.ARTIFACT_STREAM_ENABLED = _coerce_bool(
            performance.get(
                "artifact_stream_enabled",
                performance.get("writing_stream_enabled", True),
            )
        )
        self.ARTIFACT_STREAM_MAX_CHARS = int(
            performance.get(
                "artifact_stream_max_chars",
                performance.get("writing_stream_max_chars", 200_000),
            )
        )
        artifact_gen = performance.get("artifact_generation") or performance.get("writing_generation") or {}
        if not isinstance(artifact_gen, dict):
            artifact_gen = {}
        self.ARTIFACT_STREAM_MAX_RETRIES = int(artifact_gen.get("stream_max_retries", 2))
        self.ARTIFACT_STREAM_MAX_DURATION_SEC = int(
            artifact_gen.get("max_stream_duration_sec", 600)
        )
        self.ARTIFACT_STREAM_MAX_ACCUMULATED_CHARS = int(
            artifact_gen.get("max_accumulated_chars", 120_000)
        )
        self.ARTIFACT_BUFFER_STALL_CHARS = int(artifact_gen.get("buffer_stall_trace_chars", 8000))
        self.ARTIFACT_BUFFER_STALL_PREVIEW_CHARS = int(
            artifact_gen.get("buffer_stall_preview_chars", 200)
        )
        self.ARTIFACT_PARTIAL_ON_DISCONNECT = _coerce_bool(
            artifact_gen.get("partial_commit_on_disconnect", True)
        )
        ...
        self.RAG_CHUNK_ENABLED = _coerce_bool(rag.get("chunk_enabled", True))
        self.RAG_CHUNK_MIN_CHARS = int(rag.get("chunk_min_chars", 600))
        self.RAG_CHUNK_MAX_CHARS = int(rag.get("chunk_max_chars", 1200))
        self.RAG_CHUNK_OVERLAP_CHARS = int(rag.get("chunk_overlap_chars", 150))
        self.RAG_CHUNK_MAX_CHARS_WRITING = int(rag.get("chunk_max_chars_writing", 1000))
        self.RAG_CHUNK_OVERLAP_CHARS_WRITING = int(rag.get("chunk_overlap_chars_writing", 120))
        self.RAG_CHUNK_MAX_CHARS_CODE = int(rag.get("chunk_max_chars_code", 900))
        self.RAG_CHUNK_OVERLAP_CHARS_CODE = int(rag.get("chunk_overlap_chars_code", 80))
        self.RAG_CHUNK_ADJACENCY_ENABLED = _coerce_bool(rag.get("chunk_adjacency_enabled", True))
        self.RAG_CHUNK_ADJACENCY_RADIUS = int(rag.get("chunk_adjacency_radius", 1))
