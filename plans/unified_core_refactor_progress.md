...
## I. 提交建议（解决“工作区不支持提交”）
当前工作区混有**预存 WIP + 本轮重构**，建议分两笔提交：
1. 先单独提交/暂存预存 revision-intent WIP（`router.py`、`planning_node.py`、`turn_contract.py` 等既有改动 + 新增的 `reasoning_execution_guard.py`/`edit_scope.py`/`revision_loop_guard.py` 等）。
2. 再提交本轮“统一内核第一阶段”（`app/domain/action.py`、删除的 17+8 文件、`graph_runner`/`session_controller`/`graph_cache`/`mission_executor`/`conftest` 去缠绕、`scripts/smoke_imports.py`、本进度文档）。

> 若两堆改动已难以拆分，可先整体打一个 `wip:` 提交固化现状，再在新分支上按本细则推进，避免继续纠缠。
